...
const CardTemplates = ({ img, name, title, button }) => {
  return (
    <Card className="bg-white dark:bg-secondaryDark rounded-md shadow-[0_4px_24px_#00000014] border-0 flex flex-col items-center p-4 md:p-5 lg:p-6 transition-all h-full">
      ...
      <CardContent className="flex flex-col items-center  px-0 flex-1 w-full">
        ...
        <div className="text-sm md:text-base text-black dark:text-gray-200 font-medium  text-center leading-snug">
          {title}
        </div>
      </CardContent>
      <CardFooter className="w-full flex justify-center pb-4 px-0">
        <Button
          className="bg-main text-white text-base md:text-lg font-bold rounded-xl w-full py-2 md:py-3 shadow-none hover:bg-[#0ea7b5] transition-all"
          size="lg">
          {button}
        </Button>
      </CardFooter>
